...
def gshank(start,dela,suminc,nans,seed,ibk,bk,delb,zph,rho,k1,k2,jh): #return suminc
    ...
    for intx in numpy.arange(1, MAXH+1): #TODO: test interval
        ...
        for j in numpy.arange(nans):  # for( i = 0; i < nans; i++ )
            ...
            #   #for( j = jm-1; j < intx; j++ )
            for k in numpy.arange(km,intx):
                brk = 0 # brk = FALSE;
                #for (i = 0; i < nans; i++ )
                for j in numpy.arange(nans):
                    a1 = q2[j,k] #a1=q2[i][j];
                    den = (numpy.abs(numpy.real(a1)) + numpy.abs(numpy.imag(a1)))*CRIT  #den=(fabs(creal(a1))+fabs(cimag(a1)))*CRIT;
                    if den < denm:
                        den = denm
...
def rom1(n,nx,zph,rho,k1,k2,a,b,jh):
    ...
    while True:  # while( TRUE )
        ...
        for j in numpy.arange(n):
            ...
            # test convergence of 5 point romberg result
            tr, ti = test(numpy.real(t11), numpy.real(t20[j]),numpy.imag(t11), numpy.imag(t20[j]), 0)  #TODO: TEST WHY I NOT J
            if (tr > CRIT) or (ti > CRIT):
                nogo = 1 # nogo = TRUE;
...
def saoa(t,zph,rho,k1,k2,a,b,jh):
    ...
    den2 = cksm*dgam/(cgam2*(pow2(k1)*cgam2 + pow2(k2)*cgam1))  # den2=cksm*dgam/(cgam2*(ck1sq*cgam2+ck2sq*cgam1))
...
def evlua(zph, rho, k1, k2):
    ...
    if zph >= 2*rho:
        ...
        delta = (-1 + slope*1j)*delt/numpy.sqrt(1 + slope^2)
