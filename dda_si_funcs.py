...
def reflection_Rjk(k1,k2,r_j,r_k,S):
    ...
    Rjk = -(Sjk + pow2(k0)*(pow2(k1)-pow2(k2))/(pow2(k1)+pow2(k2))*numpy.exp(1j*k0*rIjk)/rIjk*Gjk)  # %as per Schmehl's thesis
...
def evanescent_k_e(theta_1,n1,n2):
    ...
    theta_c = numpy.arcsin(n2/n1)  # critical angle
    ...
    if numpy.isreal(theta_2):
        k2 = [0, n1*k0*numpy.sin(theta_1), 1j*k0*numpy.sqrt(pow2(n1*numpy.sin(theta_1)) - pow2(n2))]
    else:
        k2 = [0, n1*k0*numpy.sin(theta_1), -1j*k0*numpy.sqrt(pow2(n1*numpy.sin(theta_1)) - pow2(n2))]

    ep = [0, 1j*numpy.sqrt(pow2(n1/n2*numpy.sin(theta_1)) - 1), n1/n2*numpy.sin(theta_1)]
    es = [1, 0, 0]
...
def evanescent_E(E1s,E1p,theta_1,n1,n2):
    ...
    T2s = (2*n1*numpy.cos(theta_1))/(n1*numpy.cos(theta_1) + numpy.sqrt(pow2(n2) - pow2(n1*numpy.sin(theta_1))))*E1s
    T2p = (2*n1*numpy.cos(theta_1))/(n2*numpy.cos(theta_1) + (n1/n2)*numpy.sqrt(pow2(n2) - pow2(n1*numpy.sin(theta_1))))*E1p
...
def E_sca_SI(k,r,P,det_r,theta,phi,n1):
    ...
    for pt in numpy.arange(pts):
        ...
        for j in numpy.arange(N):
            ...
            rj = r[j,:]
            rIj = [rj[1], rj[2], -rj[3]]
            E[pt,:] = E[pt,:] + numpy.exp(-1j*numpy.dot(k_sca, rj))*(numpy.dot(Pj, e1p) * e1p + numpy.dot(Pj, e2p) * e2p) + numpy.exp(-1j*numpy.dot(k_Isca, rj))*(refl_TM*numpy.dot(Pj, e1p)*e1p + refl_TE*numpy.dot(Pj, e2p)*e2p)
